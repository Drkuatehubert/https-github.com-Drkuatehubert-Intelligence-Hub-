...
func NewRouter(
	...
	// WorldMonitor Universal API Proxy
	wmApi := router.Group("/api")
	wmApi.Use(noCacheMiddleware())
	{
		wmApi.GET("/rss-proxy", hubService.GetRSS)
		wmApi.GET("/military/v1/list-military-flights", hubService.ListMilitaryFlights)
		wmApi.Static("/geodata", "../geodata")
		wmApi.Any("/*path", hubService.ProxyJSON)
	}
	...
	publicGroup.Use(authMiddleware.TryAuth)
	{
		...
		setHubGroup(publicGroup, hubService)
	}
...
func setHubGroup(parent *gin.RouterGroup, svc *services.HubService) {
	// Compatibility with WorldMonitor legacy API
	parent.GET("/rss-proxy", svc.GetRSS)

	hubGroup := parent.Group("/hub")
	{
		hubGroup.GET("/rss", svc.GetRSS)
		hubGroup.GET("/proxy", svc.ProxyJSON)
		hubGroup.GET("/apis", svc.ListAPIs)
		hubGroup.GET("/flights", svc.GetFlights)
		hubGroup.GET("/vessels", svc.GetVessels)
		hubGroup.GET("/nearby", svc.GetNearbySignals)
	}
}
